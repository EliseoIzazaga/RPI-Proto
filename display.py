...
if __name__ == '__main__':
    ...
    try:
        ...
        device.Clear()
        device.DrawString(0,0, "LAUNCHING APP:", COLOR_GREEN)
        time.sleep(4)
        device.DrawString(0,10, "TAE001", COLOR_BLUE)
        time.sleep(5)
        device.DrawString(0,20, "WORKING...", COLOR_WHITE)
        device.SetScrollMode(H_SCROLL_DISABLE, V_SCROLL_ENABLE)
        device.EnableScrollMode(True)
        device.DrawString(0,0, "LAUNCHING APP:", COLOR_BLACK)
        device.DrawString(0,10, "TAE001", COLOR_BLACK)
        time.sleep(1)
        #device.DrawString(0,0, "LAUNCHING APP:", COLOR_BLACK)
        #device.DrawString(0,10, "TAE001", COLOR_BLACK)
        device.EnableScrollMode(False)
        device.DrawString(0,20, "WORKING...", COLOR_BLACK)
        device.DrawString(0,0, "WORKING...", COLOR_WHITE)
        device.SetScrollMode(H_SCROLL_DISABLE, V_SCROLL_ENABLE)
        #device.DrawString(0,4, "Part2", COLOR_BROWN)
        #device.DrawString(0,6, "Part3", COLOR_GOLDEN)
        #device.DrawString(0,8, "Part4", COLOR_RED)
